...
function Analysis() {
  ...
  // Fetch analysis content from backend API
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await contentAPI.getAnalysisContent();
      ...
      <div className={`max-w-[1200px] mx-auto p-10 rounded-2xl border transition-all duration-300 ${isDarkMode ? 'bg-[#1e293b] border-[#334155] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.3),0_2px_4px_-1px_rgba(0,0,0,0.2)]' : 'bg-[#ffffff] border-[#e2e8f0] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-1px_rgba(0,0,0,0.06)]'}`}>
        ...
        {loading ? (
          ...
          <>
            ...
            >
              {content?.title || '📊 Market Gap Analysis Methodology'}
            </motion.h1>
            <motion.p 
            ...
            >
              {content?.description || 'Understanding how we identify business opportunities using data-driven analysis for'} {districtName}
            </motion.p>
