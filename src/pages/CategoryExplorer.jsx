    ...
    <div className={`min-h-[calc(100vh-120px)] px-3 sm:px-4 py-4 sm:py-8 transition-colors ${b('bg-gray-50', 'bg-[#0f172a]')}`}>
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className={`text-xl sm:text-2xl font-bold ${b('text-gray-900', 'text-white')}`}>Business Category Explorer</h1>
            <p className={`text-xs sm:text-sm ${b('text-gray-500', 'text-gray-400')}`}>Browse all business categories by opportunity scores</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {paginatedCategories.map((cat, i) => (
              <motion.div key={cat._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                className={`rounded-xl border p-4 sm:p-5 ${b('bg-white border-gray-200', 'bg-[#1e293b] border-[#334155]')}`}>
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className={`font-bold text-sm sm:text-base ${b('text-gray-900', 'text-white')}`}>{cat.name}</h3>
                      <span className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full ${cat.gap >= 45 ? 'bg-green-100 text-green-700' : cat.gap >= 30 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
