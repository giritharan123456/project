...
function DataFilter({ pincodeData, filters, onFiltersChange, onClear, resultCount, totalCount }) {
  ...
  // Click outside to close — use pointerdown to avoid race condition
  useEffect(() => {
    if (!isOpen) return;
    const handler = (e) => {
  ...
  const labelCls = `text-[11px] font-bold uppercase tracking-wider mb-1.5 block ${
  ...
  return (
    <>
      ...
      >
        ...
        {hasActiveFilters && (
          <span className="px-2 py-0.5 rounded-full bg-white/25 text-[11px] font-extrabold">{getActiveCount()}</span>
        )}
      </button>

      {/* Dropdown via Portal — avoids overflow clipping */}
      {isOpen && createPortal(
        <>
          {/* Backdrop */}
          ...
          <div
            ...
            className={`fixed z-[100] p-5 rounded-2xl border-2 shadow-2xl w-[360px] sm:w-[420px] ${
