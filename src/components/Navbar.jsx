      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-14 lg:h-16 gap-4">
          ...
          <Link to="/home" className="flex items-center gap-2 no-underline flex-shrink-0" onClick={closeMenus}>
            ...
            <span className="text-sm lg:text-xl font-bold bg-gradient-to-r from-[#2563eb] to-[#7c3aed] bg-clip-text text-transparent whitespace-nowrap hidden sm:block">
              MarketVision AI
            </span>
          </Link>
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-2xl mx-1 sm:mx-2 md:mx-4 relative" ref={searchRef}>
            <div className={`flex w-full rounded-lg overflow-hidden border-2 ${b('border-gray-300 focus-within:border-[#2563eb]', 'border-slate-600 focus-within:border-[#2563eb]')} focus-within:ring-2 focus-within:ring-[#2563eb]/30 transition-all`}>
              <input
                ...
                className={`flex-1 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 text-xs sm:text-sm outline-none border-none min-w-0 ${b('bg-white text-gray-900 placeholder-gray-400', 'bg-slate-800 text-white placeholder-slate-400')}`}
              />
              {searchQuery && (
                <button type="button" onClick={() => { setSearchQuery(''); setSearchSuggestions([]); setShowSuggestions(false); }}
                  className={`px-1.5 sm:px-2 text-sm ${b('text-gray-400 hover:text-gray-600', 'text-slate-500 hover:text-slate-300')}`}>
                  ✕
                </button>
              )}
              <button type="submit" className="px-2 sm:px-3 md:px-5 py-1.5 sm:py-2 bg-gradient-to-r from-[#2563eb] to-[#7c3aed] text-white text-xs sm:text-sm font-semibold hover:opacity-90 transition-opacity whitespace-nowrap">
                Search
              </button>
            </div>
